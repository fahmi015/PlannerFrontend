                ...
                <div class="overflow-hidden">
                    <div class="bg-white dark:bg-gray-800 py-4 px-3 sm:px-6">
                        <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                <label for="user-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Username</label>
                                <input v-model="username" type="text" name="user-name" id="user-name" autocomplete="username" class="dark:bg-gray-700 dark:text-gray-300 dark:border-0 mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>

                            <div class="col-span-6 sm:col-span-3">
                                <label for="first-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">First name</label>
                                <input v-model="first_name" type="text" name="first-name" id="first-name" autocomplete="first name" class="dark:bg-gray-700 dark:text-gray-300 dark:border-0 mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
            
                            <div class="col-span-6 sm:col-span-3">
                                <label for="last-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Last name</label>
                                <input v-model="last_name" type="text" name="last-name" id="last-name" autocomplete="last name" class="dark:bg-gray-700 dark:text-gray-300 dark:border-0 mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
...
<script setup>
    ...
    const [first_name, last_name,username,avatar] = form_update_profile.useFieldModel(['first_name', 'last_name', 'username','avatar']);
    ...
    const changePassword = form_change_password.handleSubmit( async (values) => {
        ...
        let result = await userModel.updatePassword(auth.user.id, values)
        if(result.status){
            current_password.value = ''
            new_password.value = ''
            confirm_new_password.value = ''
    ...
    const updateProfile = form_update_profile.handleSubmit( async (values) => {
        alertModel.clear()
        IsSubmitting.value = true
        let result = await userModel.update(auth.user.id, values)
        if(result.status){
